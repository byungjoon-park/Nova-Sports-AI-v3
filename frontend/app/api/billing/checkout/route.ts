...
export async function POST(request: Request) {
  ...
  if (!secret) {
    return NextResponse.json(
      { error: "결제 시스템이 아직 연결되지 않았습니다. STRIPE_SECRET_KEY를 설정하세요." },
      { status: 503 },
    );
  }
  ...
  if (!priceId && !useDynamicAmount) {
    return NextResponse.json(
      { error: "선택한 구독 플랜의 Stripe Price ID 또는 유효한 월 금액이 설정되지 않았습니다." },
      { status: 400 },
    );
  }
  ...
  form.set("success_url", `${origin}/billing?billing=success`);
  form.set("cancel_url", `${origin}/billing?billing=cancelled`);
  form.set("locale", "ko");
  ...
  if (!response.ok || typeof data.url !== "string") {
    return NextResponse.json(
      { error: data?.error?.message || "결제 세션을 만들지 못했습니다." },
      { status: 502 },
    );
  }
