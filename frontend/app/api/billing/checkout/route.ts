...
export async function POST(request: Request) {
  ...
  const body = await request.json().catch(() => null) as {
    plan?: string; email?: string; athleteId?: string; parentEmail?: string; parentName?: string; returnPath?: string;
  } | null;
  const plan = body?.plan && PRICE_ENV[body.plan] ? body.plan : "pro";
  ...
  const safeReturnPath = body?.returnPath === "/mobile/billing" ? "/mobile/billing" : "/billing";
  ...
  if (useDynamicAmount) {
    ...
    form.set("line_items[0][price_data][product_data][name]", plan === "team" ? "NOVA Sports AI Team" : "NOVA Sports AI Pro");
  } else form.set("line_items[0][price]", priceId as string);
  ...
  if (!response.ok || typeof data.url !== "string") return NextResponse.json({ error: data?.error?.message || "결제 세션을 만들지 못했습니다." }, { status: 502 });
